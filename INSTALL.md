# PCAP Raptor instalation procedures 
...
url=$(curl -s https://api.github.com/repos/helviojunior/pcapraptor/releases | jq -r '[ .[] | {id: .id, tag_name: .tag_name, assets: [ .assets[] | select(.name|match("linux-amd64.tar.gz$")) | {name: .name, browser_download_url: .browser_download_url} ]} | select(.assets != []) ] | sort_by(.id) | reverse | first(.[].assets[]) | .browser_download_url')

cd /tmp
rm -rf pcapraptor-latest.tar.gz pcapraptor
wget -nv -O pcapraptor-latest.tar.gz "$url"
tar -xzf pcapraptor-latest.tar.gz

rsync -av pcapraptor /usr/local/sbin/
chmod +x /usr/local/sbin/pcapraptor
...
url=$(curl -s https://api.github.com/repos/helviojunior/pcapraptor/releases | jq -r --arg filename "darwin-${arch}.tar.gz\$" '[ .[] | {id: .id, tag_name: .tag_name, assets: [ .assets[] | select(.name|match($filename)) | {name: .name, browser_download_url: .browser_download_url} ]} | select(.assets != []) ] | sort_by(.id) | reverse | first(.[].assets[]) | .browser_download_url')

cd /tmp
rm -rf pcapraptor-latest.tar.gz pcapraptor
curl -sS -L -o pcapraptor-latest.tar.gz "$url"
tar -xzf pcapraptor-latest.tar.gz
...
```
# Download latest helviojunior/pcapraptor release from github
function Invoke-Downloadpcapraptor {

    $repo = "helviojunior/pcapraptor"
    ...
    $file = "pcapraptor-latest$extension"
    ...
    Write-Host "Getting pcapraptor version banner"
    . $name version 
}
...
Function Get-AssetData {
    ...
    if (Get-Member -inputobject $Release -name "assets" -Membertype Properties) {
        ...
        {
            If ($asset.name.Contains("pcapraptor-") -and $asset.name.Contains("$platform-$arch$extension")) { Return $asset }
        }
...
```
git clone https://github.com/helviojunior/pcapraptor.git
cd pcapraptor
go get ./...
go build
```
...
```
go install .
ln -s /root/go/bin/pcapraptor /usr/bin/pcapraptor
```
